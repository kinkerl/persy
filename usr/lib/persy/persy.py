...
class TheSyncer(Thread):
	...
	def run(self):
		...
		while True:
			...
			if USE_REMOTE and tick >= (self.sleep_remote/self.sleep_local) :
				tick = 0
...
if __name__ == "__main__":
	...
	parser.add_option("--username", dest="username", default="", help="sets username")
	parser.add_option("--useremail", dest="useremail", default="", help="sets useremail")
	(options, args) = parser.parse_args()
	#configuration
	global config
	#create programdirectory
	if not os.path.isdir("%s/%s"%(USERHOME,USERHOME_FOLDER)):
		os.popen("mkdir %s/%s"%(USERHOME,USERHOME_FOLDER))
	config = ConfigObj("%s/%s/config"%(USERHOME,USERHOME_FOLDER))
	
	USE_REMOTE = config['remote']['use_remote']=='True'
	...
	if options.config:
		...
		if options.useremail:
			config['local']['useremail'] = options.useremail
		if options.username:
			config['local']['username'] = options.username
		if options.add_dir:
